...
export const riderProfileAction = async (id, status) => {
  try {
    const docRef = doc(db, "ridersprofile", id);
    await updateDoc(docRef, {
      status,
    });
  } catch (e) {
  ...
  revalidatePath("/dashboard/rider-profile");
  redirect("/dashboard/rider-profile");
};

export const assignAreaToRider = async (id, data) => {
  try {
    const docRef = doc(db, "ridersprofile", id);
    await updateDoc(docRef, {
      assignedArea: data,
  ...
  revalidatePath("dashboard/assign-areas");
};
